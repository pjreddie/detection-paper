...
\end{center}
\label{combine}
\caption{\textbf{Model combination expermients on VOC 2007.} }
\end{table}

The best Fast R-CNN model achieves a mAP of 71.8\% on the VOC 2007 test set. When combined with YOLO, its mAP increases by 2.9\% to 74.7\%. We also tried combining the top Fast R-CNN model with several other training versions of Fast R-CNN. Those ensembles produced small increases in mAP between .3 and .6\%, see table \ref{combine} for details.
...
We introduce YOLO, a unified detection pipeline. YOLO is extremely fast and moderately accurate.
